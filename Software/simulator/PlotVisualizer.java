...
public class PlotVisualizer implements Runnable
{
	...
	private static double stepsPerMM = stepsPerRotation/spoolCirc; // what is MM? movement?
	...
	private static void readLoop() throws IOException, InterruptedException {
		...
		img = new BufferedImage(disparity, 2*disparity, BufferedImage.TYPE_INT_RGB); // BufferedImage(int width, int height, int imageType)
		...
		synchronized(stepMonitor) {
			...
			leftStepper = new Stepper(currLeft, 7,8,9,10); // TODO: fix coupling!
			rightStepper = new Stepper(currRight, 2,3,5,6);
		}

		while(true) {
			String str = br.readLine();
			if(str == null) {
				// no more outputted data from ./sim so save to file
				try {
					// retrieve image
			...
			if(servoPos == 20) {
				...
				// for loop adds "thickness" to drawing (more than just 1 pixel for the end of the marker)
				for(int y=-1;y<2;y++) for(int x=-1;x<2;x++)
					// sets pixel in buffered image
					img.setRGB((int)pos[0]+x,(int)pos[1]+y,0xffffffff); // white marker
			}
		}
	}
}
...
class Stepper {
	...
	void setPin(int pin, boolean set) {
		int prevPos = step & 0x7; // & 0b0111 -> step will only have non-zeros for first 3 bits

		for(int i=0 ; i<4 ; i++) {
			if(pins[i] == pin) {
				int orBit = 0b1000 >> i; // shift bits i times
				int orMask = 0b1111 ^ orBit;
				bitState = (bitState & orMask); // extract the lower 4 bits of bitState
				if(set) {
					bitState = (bitState | orBit);
				}
				int pos = 0;
				switch(bitState) {
					case 0b1000:
