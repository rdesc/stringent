...
{  
  if(currentlySelectedPlot != plotNo) {
    ...
    if(svgFile) {
      fclose(svgFile);
      svgFile = fopen("dummy_fail_this_open", FILE_READ);;      
    }
    
    char* svgName = (char*)strdup("1.svg");
    svgName[0] = '0'+plotNo;    
    svgFile = fopen(svgName, FILE_READ);
    if(svgFile) {
        //found svg
      SER_PRINT("Found: ");      
      SER_PRINTLN(svgName);      
...
#ifdef HAS_BATTERY_MEASUREMENT  
#ifndef USE_DATA_FROM_DISK
  batteryFile = fopen("battery.log", FILE_WRITE);;      
#endif
#endif //HAS_BATTERY_MEASUREMENT
}
...
seekToPathStart(bool rewindFile) {
  if(rewindFile) {
    fseek(svgFile, 0, SEEK_SET);  //rewind (we could have paused)
  }
    if(!seekTo((char*)"<path")) {
...
getNextPathSegment(float *x, float *y, int *line, bool first)
{
  ...
  while(true) {
    ...
    if(c == 'M') {
      ...
      *line = 0;     
      fgetc(svgFile); //eat a space byte
      rel = true;
    }
    else if(c == 'L') {
      *line = 1;
      ...
      *line = 1;
      ...
      tmp_seek(svgFile, ftell(svgFile)-1);       
    }
    else {
